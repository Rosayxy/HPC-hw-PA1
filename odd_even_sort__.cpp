...
void Worker::sort()
{
  ...
  float buf = 0;
  int ceil=(n+nprocs-1)/nprocs;
  float *tmp = new float[ceil+1]; // todo 为了兼容性改一下
  float* buf1=new float[ceil+1];
  //MPI_Request request;
  //MPI_Request request1;
  
  int next_len=((rank==nprocs-2)?(n-(nprocs-1)*ceil):(ceil));
  int prev_len=((rank==0)?0:ceil);
  if(n%2==0){
  for (int ii = 0; ii < (nprocs ) / 2; ii++)
  {
    ...
    if (rank % 2 == 0)
    {
      ...
      if (data[block_len - 1] > buf)
      {
        ...
        if(cnt<block_len){ // 一定是start1_ptr 对应的用完了
    ...
    // 奇数个块
    for(int ii=0;i<(nprocs+1)/2;i++){
      // 偶数排序
    //MPI_Barrier(MPI_COMM_WORLD);
    if ((rank % 2 == 0)&&(rank!=nprocs-1))
    {
      // int MPI_Sendrecv(MPICH2_CONST void *sendbuf, int sendcount, MPI_Datatype sendtype,int dest, int sendtag,void *recvbuf, int recvcount, MPI_Datatype recvtype,int source, int recvtag,MPI_Comm comm, MPI_Status *status)
